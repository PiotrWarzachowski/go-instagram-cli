...
func messagesAction(ctx context.Context, cmd *cli.Command) error {
	...
	c, err := storage.RestoreClient(stored)
	if err != nil {
...
func runInteractiveMode(c *client.Client, storage *client.SessionStorage) error {
	...
	for {
		...
		case "r", "refresh":
			...
			conversations, fromCache = getConversationsWithCache(c, storage, true) // Force refresh
			clearScreen()
			continue
		case "":
...
func getConversationsWithCache(c *client.Client, storage *client.SessionStorage, forceRefresh bool) ([]client.Conversation, bool) {
...
func displayConversations(conversations []client.Conversation) {
...
func openConversation(c *client.Client, conv client.Conversation, reader *bufio.Reader) error {
	clearScreen()
...
func displayMessages(messages []client.Message, myUserID int64) {
	if len(messages) == 0 {
		fmt.Printf("\n%s📭 No messages in this conversation.%s\n", colorDim, colorReset)
		return
	}

	// Sort messages by time (oldest first for natural reading)
...
func displayMyMessage(msg client.Message, timeStr string) {
...
func displayTheirMessage(msg client.Message, timeStr string) {
	// Format message text with wrapping
	lines := wrapText(msg.Text, 45)
